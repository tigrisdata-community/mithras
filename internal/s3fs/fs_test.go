...
var (
	endpoint = flag.String("endpoint", "", "s3 endpoint (empty uses AWS default)")
...
// Compile-time assertions for the write interfaces.
var (
	_ s3fs.CreateFS    = (*s3fs.S3FS)(nil)
	_ s3fs.WriteFileFS = (*s3fs.S3FS)(nil)
	_ s3fs.RemoveFS    = (*s3fs.S3FS)(nil)
	_ s3fs.MkdirAllFS  = (*s3fs.S3FS)(nil)
)
...
func TestWriteFile(t *testing.T) {
	...
	testFs := s3fs.New(wrappedCl, *bucket)
	const name = "writefile/hello.txt"
	payload := []byte("hello from WriteFile")

	if err := testFs.WriteFile(name, payload, 0644); err != nil {
		t.Fatal(err)
	}

	got, err := fs.ReadFile(testFs, name)
	if err != nil {
...
func TestCreateStreaming(t *testing.T) {
	...
	testFs := s3fs.New(wrappedCl, *bucket)
	const name = "stream/out.bin"

	w, err := testFs.Create(name)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < 16; i++ {
		chunk := bytes.Repeat([]byte{byte('a' + i%26)}, 4096)
		want.Write(chunk)
	...
	if !bytes.Equal(got, want.Bytes()) {
		t.Fatalf("payload mismatch: want %d bytes, got %d", want.Len(), len(got))
	}
}
...
func TestRemove(t *testing.T) {
	...
	testFs := s3fs.New(wrappedCl, *bucket)
	const name = "remove/me.txt"
	if err := testFs.WriteFile(name, []byte("bye"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if err := testFs.Remove(name); err != nil {
		t.Fatal(err)
	}

	if _, err := testFs.Stat(name); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("want fs.ErrNotExist after remove, got %v", err)
	}
}
...
func TestMkdirAll(t *testing.T) {
	...
	testFs := s3fs.New(wrappedCl, *bucket)
	if err := testFs.MkdirAll("a/b/c", 0755); err != nil {
		t.Fatal(err)
	}

	// An empty directory should still be listable and not appear as a file.
	entries, err := testFs.ReadDir("a/b/c")
	if err != nil {
		t.Fatalf("readdir a/b/c: %v", err)
	}
	if len(entries) != 0 {
		t.Fatalf("want empty directory a/b/c, got %d entries: %+v", len(entries), entries)
	}

	// Intermediate directory "a/b" should list "c" as a directory and
	// must not list the marker object "c/" itself as a file entry.
	entries, err = testFs.ReadDir("a/b")
	if err != nil {
		t.Fatalf("readdir a/b: %v", err)
	}
	if len(entries) != 1 {
		t.Fatalf("want exactly [c], got %d: %+v", len(entries), entries)
	}
	if entries[0].Name() != "c" || !entries[0].IsDir() {
		t.Fatalf("want dir entry 'c', got %q (isDir=%v)", entries[0].Name(), entries[0].IsDir())
	}
}
